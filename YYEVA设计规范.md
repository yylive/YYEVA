...
## **导出工具** 使用介绍 
   ...
   * 普通资源转换
   * 动态元素MP4转换
   内部会根据选择的合成是否有引用mask_合成来判断选择哪种转换模式输出资源
...
### 动态元素MP4转换 
   ...
   之前有提到过，动态元素MP4是在透明MP4的基础上，开发的一个功能，所以我们选择的输出合成，必须是一个左边RGB，右边Alpha的画布。如果不是，内部会帮用户使用白色(#ffffff)创建一个蒙版效果，来自动生成一个这样的合成，所以当您的合成可以使用效果白色(#ffffff)创建一个蒙版生成alpha区域的时候，可以不需要去制作这样一个一个左边RGB，右边Alpha的合成，我们内部会帮您去创建
